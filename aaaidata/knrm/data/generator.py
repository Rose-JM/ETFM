...
class DataGenerator(Configurable):
    ...
    def pairwise_reader(self, pair_stream, batch_size, with_idf=False):
        l_q, l_d, l_d_aux, l_idf, l_y = [], [], [], [], []

        # 先统计一共有多少行（用于设置总步数）
        lines = pair_stream
        total_lines = len(lines)
        #pair_stream.seek(0)  # 回到文件开头

        print(f"Total {total_lines} lines detected. Start reading with batch size {batch_size}...")

        for line_num, line in enumerate(tqdm(lines, total=total_lines, desc="Reading Pairs", ncols=100), start=1):
            cols = line.strip().split('\t')

            if len(cols) < 4:
                print(f"[Warning] Line {line_num}: Less than 4 columns, skipped.")
                continue

            if not all(col.strip() for col in cols[:4]):
                print(f"[Warning] Line {line_num}: Empty columns detected, skipped.")
                continue

            try:
                y = float(cols[3])
            except ValueError:
                print(f"[Warning] Line {line_num}: Invalid label {cols[3]}, skipped.")
                continue
            ...
            # 自动分隔符选择
            sep_q = ',' if ',' in cols[0] else None
            sep_t1 = ',' if ',' in cols[1] else None
            sep_t2 = ',' if ',' in cols[2] else None

            try:
                q = np.array(
                    [int(t) for t in cols[0].split(sep_q) if t.strip().isdigit() and int(t) < self.vocabulary_size])
                t1 = np.array(
                    [int(t) for t in cols[1].split(sep_t1) if t.strip().isdigit() and int(t) < self.vocabulary_size])
                t2 = np.array(
                    [int(t) for t in cols[2].split(sep_t2) if t.strip().isdigit() and int(t) < self.vocabulary_size])
            except ValueError:
                print(f"[Warning] Line {line_num}: Non-integer token detected, skipped.")
                continue

            if q.size == 0 or t1.size == 0 or t2.size == 0:
                print(f"[Warning] Line {line_num}: Empty sequence after parsing, skipped.")
                continue
            ...
            # padding
            v_q = np.ones(self.max_q_len, dtype=int) * -1
            v_d = np.ones(self.max_d_len, dtype=int) * -1
            v_d_aux = np.ones(self.max_d_len, dtype=int) * -1

            v_q[:min(len(q), self.max_q_len)] = q[:min(len(q), self.max_q_len)]
            v_d[:min(len(t1), self.max_d_len)] = t1[:min(len(t1), self.max_d_len)]
            v_d_aux[:min(len(t2), self.max_d_len)] = t2[:min(len(t2), self.max_d_len)]
            ...
            if with_idf:
                if len(cols) >= 5:
                    sep_idf = ',' if ',' in cols[4] else None
                    try:
                        idf = np.array([float(t) for t in cols[4].split(sep_idf) if t.strip()])
                    except ValueError:
                        print(f"[Warning] Line {line_num}: Invalid IDF values, using default ones.")
                        idf = np.ones(len(q))
                else:
                    idf = np.ones(len(q))

                v_idf = np.zeros(self.max_q_len, dtype=float)
                v_idf[:min(len(idf), self.max_q_len)] = idf[:min(len(idf), self.max_q_len)]
                l_idf.append(v_idf)

            if len(l_q) >= batch_size:
                X = {
                    self.q_name: np.array(l_q, dtype=int),
                    self.d_name: np.array(l_d, dtype=int),
                    self.idf_name: np.array(l_idf, dtype=float) if with_idf else None,
                    self.aux_d_name: np.array(l_d_aux, dtype=int)
                }
                Y = np.array(l_y, dtype=int)
                yield X, Y
                l_q, l_d, l_d_aux, l_y, l_idf = [], [], [], [], []
        ...
        print("pairwise_reader finished ✅")


if __name__ == '__main__':
    from team2box.aaaidata.knrm.utils import set_basic_log, load_py_config
    set_basic_log()
    if 4 != len(sys.argv):
